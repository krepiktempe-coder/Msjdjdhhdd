...
export function Rooster({ viewMode: viewModeProp = "week" }: { viewMode?: ViewMode }) {
  ...
  // Routes filtered for the shift dialog: hides routes already assigned to the selected staff.
  // Resets to show ALL routes once the driver has covered every route at least once (cycle complete).
  const filteredRoutesForDialog = useMemo(() => {
    const resourceId = shiftForm.resourceId
    if (!resourceId || routes.length === 0) return routes

    // Exclude the shift currently being edited so it doesn't count against itself
    ...
    const assignedNames = new Set(staffRouteShifts.map(s => s.title))

    // If every route has been assigned at least once → new cycle begins, show all
    const allDone = routes.every(r => assignedNames.has(r.name))
    if (allDone) return routes

    // Otherwise hide already-assigned routes (but always keep the currently selected one)
    return routes.filter(r => !assignedNames.has(r.name) || r.name === shiftForm.title)
  ...
  // Auto-assign next unassigned route to every driver for each empty day in the current view
  const handleAutoAssignAllRoutes = async () => {
    if (!routes.length || !orderedResources.length) return
    setIsGenerating(true)
    try {
      const batchId = Date.now()
      const pending: Shift[] = []
      ...
      for (const resource of orderedResources) {
        const existingRouteShifts = shifts.filter(
          s => s.resourceId === resource.id && !OFF_LABELS.has(s.title)
        )
        const assignedNames = new Set(existingRouteShifts.map(s => s.title))

        // Queue = unassigned routes; if all done → full reset (new cycle)
        const remaining = routes.filter(r => !assignedNames.has(r.name))
        const queue: typeof routes = remaining.length === 0 ? [...routes] : [...remaining]

        // Dates in current view with no shift for this driver (also skip days already covered by pending)
        const emptyDates = colDates.filter(date => {
          const dk = toDateKey(date)
          return !shifts.some(s => s.resourceId === resource.id && s.date === dk)
            && !pending.some(s => s.resourceId === resource.id && s.date === dk)
        })

        for (const date of emptyDates) {
          if (queue.length === 0) queue.push(...routes)   // cycle reset
          const route = queue.shift()!
          const preset = getShiftPreset(route.shift ?? "")
          const color = routeEffectiveColorMap.get(route.name) ?? route.color ?? "#3B82F6"
          pending.push({
            id: `auto${batchId}_${resource.id}_${toDateKey(date)}`,
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {routes.length > 0 && shiftForm.resourceId && (() => {
                        ...
                        const assignedNames = new Set(
                          shifts.filter(s => s.resourceId === shiftForm.resourceId && !OFF_LABELS.has(s.title) && s.id !== editingShiftId)
                                .map(s => s.title)
                        )
                        const allDone = routes.every(r => assignedNames.has(r.name))
                        const remaining = routes.filter(r => !assignedNames.has(r.name)).length
